#!/usr/bin/python
# -*- coding: utf-8 -*-
# (c) 2022, Elias Boulharts (@elias-boulharts)
...
DOCUMENTATION = '''
module: vault_kv1_delete
version_added: 1.0.0
author:
  - Elias Boulharts (@elias-boulharts)
short_description: Delete a secret from HashiCorp Vault's KV version 1 secret store
requirements:
  - C(hvac) (L(Python library,https://hvac.readthedocs.io/en/stable/overview.html))
  - For detailed requirements, see R(the collection requirements page,ansible_collections.community.hashi_vault.docsite.user_guide.requirements).
description:
  - Delete a secret from HashiCorp Vault's KV version 1 secret store.
notes:
...
options:
  path:
    description:
      - Vault KV path to be deleted. Do not include secret/
    type: str
    required: True
'''

EXAMPLES = """
- name: Delete the secret/mysecret secret.
...
def run_module():
    ...
    try:
        response = client.secrets.kv.v1.delete_secret(path=path)
